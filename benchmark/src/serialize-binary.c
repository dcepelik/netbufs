...
static void serialize_i32(struct nb_buf *buf, uint32_t i32)
{
	if (i32 >= 0)
		serialize_uint(buf, 8, i32); /* TODO Ask MM: is this safe? */
	else
		serialize_uint(buf, 8, -i32 | 0x80 << 7);
}
...
{
	serialize_u8(buf, tm->tm_hour);
	serialize_u8(buf, tm->tm_min);
	serialize_u8(buf, tm->tm_sec);
}
...
static void serialize_rte_attr(struct nb_buf *buf, struct rte_attr *attr)
{
	...
	case RTE_ATTR_TYPE_BGP_AGGREGATOR:
		...
		serialize_u64(buf, attr->aggr.as_no);
		break;
