...
nb_err_t nb_bind(struct nb_group *grp, nb_lid_t lid, char *name, bool reqd);
struct nb_group *nb_group(struct nb *nb, nb_lid_t lid, char *name);

nb_err_t nb_recv_group(struct nb *nb, nb_lid_t lid);
nb_err_t nb_recv_group_end(struct nb *nb);

nb_err_t nb_recv_id(struct nb *nb, nb_lid_t *lid);
